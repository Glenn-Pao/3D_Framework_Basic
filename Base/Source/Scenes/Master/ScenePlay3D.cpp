...
CScenePlay3D::CScenePlay3D(void)
	: thePlayer(NULL)
{
}
...
//Init codes
void CScenePlay3D::ParametersInit()
{
...
void CScenePlay3D::Init(int level)
{
...
const float CScenePlay3D::GetHeightMapY(float x, float z)
{
	return TERRAIN_SCALE.y*(ReadHeightMap(m_heightMap, x / TERRAIN_SCALE.x, z / TERRAIN_SCALE.z));
}
Particle* CScenePlay3D::FetchParticle()
{
...
void CScenePlay3D::UpdateParticle(double dt)
{
	//Set initial data
	for(std::vector<Particle*> ::iterator it = m_parList.begin() ; it != m_parList.end(); ++it)
	{
...
void CScenePlay3D::UpdatePlayerStatus(const unsigned char key, const bool status)
{
	thePlayer->UpdateCameraStatus(key, camera);
}
void CScenePlay3D::UpdateCameraStatus(const unsigned char key, const bool status)
{
	camera.UpdateStatus(key);
}
void CScenePlay3D::UpdateWeaponStatus(const unsigned char key)
{
}
void CScenePlay3D::ToggleFog()
{
...
void CScenePlay3D::Update(double dt)
{
...
void CScenePlay3D::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void CScenePlay3D::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if(!mesh || mesh->textureID <= 0)
		return;
...
void CScenePlay3D::RenderMeshIn2D(Mesh *mesh, bool enableLight, float size, float x, float y)
{
	Mtx44 ortho;
...
void CScenePlay3D::RenderParticle(Particle *particle)
{
...
void CScenePlay3D::RenderMesh(Mesh *mesh, bool enableLight, bool enableFog)
{
	Mtx44 MVP, modelView, modelView_inverse_transpose;
...
void CScenePlay3D::RenderSkyPlane(Mesh* mesh, Color color, int slices, float
...
void CScenePlay3D::RenderTerrain()
{
	modelStack.PushMatrix();
